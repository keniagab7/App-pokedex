<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Lista de Pokémon</title>
    <link rel="stylesheet" href="<?= BASE_URL ?>css/estilos.css">

</head>

<body>
    <div class="container">

        <div class="header">
            <h1>Pokédex</h1>
            <img src="data:image/png;base64," alt="Pokédex Icon" class="pokedex-icon">
        </div>

        <form action="<?= BASE_URL ?>pokemon/buscar" method="GET" class="search-form">
            <input type="text" name="query" placeholder="Search by name or number" required>
            <button type="submit">Search</button>
        </form>

        <ul class="grid-container">
            <?php if (isset($datos['results']) && !empty($datos['results'])) : ?>
                <?php foreach ($datos['results'] as $pokemon) : ?>
                    <?php
                    // Obtener el ID de la url del Pokémon
                    if (isset($pokemon['id'])) {
                        $id = $pokemon['id'];
                    } else {
                        preg_match('/\/(\d+)\/$/', $pokemon['url'], $matches);
                        $id = $matches[1];
                    }
                    ?>
                    <li>
                        <a href="<?= BASE_URL ?>pokemon/detalle/<?= $pokemon['name'] ?>">
                            <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/<?= $id ?>.png" alt="<?= ucfirst($pokemon['name']) ?>">
                            <?= sprintf('#%03d', $id) ?> - <?= ucfirst($pokemon['name']) ?>
                        </a>
                    </li>
                <?php endforeach; ?>
            <?php else : ?>
                <li>No se encontraron Pokémon.</li>
            <?php endif; ?>
        </ul>
    </div>
</body>

</html>